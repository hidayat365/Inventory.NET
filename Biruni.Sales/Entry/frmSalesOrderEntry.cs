...
{
    public partial class frmSalesOrderEntry : Biruni.Shared.Templates.frmEntry2
    {
        int InvoiceID = -1;
        int PaymentID = -1;
        ...
        public frmSalesOrderEntry()
        {
            frmSalesOrderEntry_Helper(-1);
        }

        public frmSalesOrderEntry(int id)
        {
            frmSalesOrderEntry_Helper(id);
        }

        public void frmSalesOrderEntry_Helper(int id)
        {
            InitializeComponent();
            InitializeForm();
            InitializeData(id);
        ...
        private void InitializeData(int id)
        {
            ...
            try
            {
                ...
                daOrders1.FillSalesQuotationAll(dsCore2.Orders);
                // get data
                if (id < 0)
                {
                    // mode
        ...
        private void InitializeForm()
        {
            ...
            // transaction type
            ModulePrefix = TransactionTypes.TX_SALES_ORDER;
            // grid event handlers
        ...
        private void GeneratePayment(dsCore ds)
        {
            ...
            dt.Orders[0].OrderNo = DbHelper.GenerateNewOrderID(TransactionTypes.TX_SALES_PAYMENT, dt.Orders[0].OrderDate.Year);
            dt.Orders[0].OrderType = TransactionTypes.TX_SALES_PAYMENT;
        ...
        private void GenerateInvoice(dsCore ds)
        {
            ...
            // ubah jenis transaksi
            dt.Orders[0].ParentID = ds.Orders[0].ID;
            dt.Orders[0].OrderNo = DbHelper.GenerateNewOrderID(TransactionTypes.TX_SALES_INVOICE, dt.Orders[0].OrderDate.Year);
            dt.Orders[0].OrderType = TransactionTypes.TX_SALES_INVOICE;
        ...
        private bool ValidateUserInput()
        {
            ...
            {
                RibbonMessageBox.Show("Please select Customer and set Currency Type before saving data",
                    Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return false;
            }
            if (radioButton1.Checked && Convert.ToDouble(c1Label1.Value) != Convert.ToDouble(c1NumericEdit1.Value))
            {
                RibbonMessageBox.Show("Payment Value must be equals to Sales Order value \n",
                    Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return false;
            }
